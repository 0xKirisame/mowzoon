"""Tests for signals.py (L1 signal layer). Run:  python test_signals.py"""
...
class TestMaturityGates(unittest.TestCase):
    ...
    def test_young_account_shares_gate_at_28_days(self):
        # 20 days of history: shares stay absent (known-issue 1 fix - the 30d
        # window is not yet full, so the ratios would read dishonestly).
        young = [tx(d, "discretionary", 50) for d in range(0, 21, 2)]
        out = by_name(compute_signals(young, 10000, TODAY))
        self.assertNotIn("savings_rate", out)
        self.assertNotIn("lifestyle_share", out)
        # 30 days of history: shares appear; the slowest signals stay gated
        # (momentum legitimately matures here: 4 nonzero prior weeks exist).
        mature = [tx(d, "discretionary", 50) for d in range(0, 31, 2)]
        out = by_name(compute_signals(mature, 10000, TODAY))
        self.assertIn("savings_rate", out)
        self.assertIn("lifestyle_share", out)
        self.assertNotIn("runway", out)      # needs 56d span
        self.assertNotIn("anomaly", out)     # needs 20 disc tx before the window
...
class TestSavingsRate(unittest.TestCase):
    ...
    def test_oversaver_direction_above_anchor(self):
        ledger = [tx(1, "discretionary", 100)] + [tx(35, "fixed", 100)]
        out = by_name(compute_signals(ledger, 10000, TODAY))
        s = out["savings_rate"]
...
class TestRobustness(unittest.TestCase):
    def test_dirty_rows_skipped_not_fatal(self):
        ...
        _, skipped = signals.compute_signals_with_skipped(ledger, 1000, TODAY)
